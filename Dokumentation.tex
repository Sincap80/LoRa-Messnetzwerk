...
\thispagestyle{plain}
\section*{Einleitung}
LoRa ist eine neue innovative Technologie, die sehr energiesparend ist, sodass es perfekt für unsere Idee eines kommerziellen Füllstand-Systems geeignet ist.
...
In dem folgenden Bild erkennt man unseren ersten fertigen Prototypen , der den Füllstand einer Kaffeekanne anzeigt. Die Einzelteile bestehen aus einer gelöteter Platine inkl. Controller, einer Wägezelle, dem Transmitter-(LoRa-)Modul mit Antenne und einem LCD-Display. Detaillierte Informationen zu den einzelnen Bauteilen folgen in den nächsten Kapiteln. Sicherlich ist hiermit noch kein Design-Preis zu gewinnen, denn zunächst steht die valide Funktion im Vordergrund.
\begin{figure}[H]
    \center
...
\end{figure}
\noindent 
Das Ziel dieser Dokumentation ist, dass klar und verständlich erklärt wird, wie der erste Prototyp funktioniert, welche Stärken dieser besitzt und wie man diesen für den kommerziellen Einsatz verbessert.
\newpage
...
\end{itemize}
\subsection{Vereinfachte Funktionsweise für momentane Gewichtsmessung} \label{sec:vereinfachte-momentane-gewichtsmessung}
\noindent
...
Leider reicht die unter \ref{sec:vereinfachte-momentane-gewichtsmessung} beschriebene Gewichtsmessmethodik in einer nicht perfekten Welt mit elektrischen Interferenzen nicht aus, um eine zuverlässige kontinuierliche Gewichtsmessung durchführen zu können. Wobei für uns folgende zwei Arten von Interferenzen wichtig sind: 

\subsubsection{Rauschen}
Wenn wir das Gewicht fein genug messen, sehen wir (in Abbildung \ref{fig:waegezelle-rauschen}) dass der Gewichtswert in einem bestimmten Bereich hin und her schwankt, selbst wenn wir das Gewicht auf der Wägezelle nicht verändern. 
\begin{figure}[H]
    \center
    \includegraphics[width=15cm]{Bilder/waegezelle-rauschen.png}\\
    \caption{Gemessene Werte einer Wägezelle in einem Koordinatensystem bei konstanter Belastung}
    \begin{center} \quelle\url{https://phidgets.wordpress.com/2014/05/12/noise-and-drift-in-load-cells/} \end{center}
    \label{fig:waegezelle-rauschen}
\end{figure}

\subsubsection{Drift}
Die Verschiebung des Messbereiches durch Temperaturveränderung.
\begin{figure}[H]
    \center
    \includegraphics[width=15cm]{Bilder/waegezelle-drift.png}\\
    \caption{Gemessene Werte einer Wägezelle in einem Koordinatensystem bei Temperaturveränderung}
    \begin{center} \quelle\url{https://phidgets.wordpress.com/2014/05/12/noise-and-drift-in-load-cells/} \end{center}
    \label{fig:waegezelle-drift}
\end{figure}
\noindent
Um dieses Problem zu lösen, nutzen wir einen Algorithmus der wie folgt funktioniert:\\
Das Differenzgewicht ~--~ von der letzten und der momentanen Gewichtsmessung ~--~ wird mit einer zuvor gewählten Veränderungsrate (z.B. 0.005 für 5 Gramm) verglichen. Wenn das Differenzgewicht höher als die gewählte Veränderungsrate ist, wird der momentan gemessene Wert verwendet und angezeigt, im anderen Fall wird der Wert verworfen, weil er vermutlich durch Rauschen oder Drift verursacht wurde.\\
\\
Als Programmablauf könnte das so aussehen:
\begin{itemize}
    \item Alte Gewichtsmessung = 0.200 (200 Gramm)
    \item Neue Gewichtsmessung = 0.199 (199 Gramm)
    \item Gewichtsdifferenz: 0.200 - 0.199 = 0.001 (1 Gramm)
    \item Veränderungsrate = 0.005 (5 Gramm)
    \item -0.005 < 0.001 < 0.005 = true
    \item neue Gewichtsmessung wird verworfen und alter Gewichtswert bleibt erhalten
\end{itemize}
Der Veränderungswert gibt auch die kleinstmögliche messbare Veränderung an. Am Beispiel unseres Projektes könnte bei einem Veränderungswert von ca. 5 Gramm, jemand mit einem Strohhalm Flüssigkeit aus dem Behältnis saugen, ohne dass das Programm die Gewichtsveränderung wahrnimmt ~--~ was aber in der Praxis zu vernachlässigen ist.
\subsection{Algorithmus für Gewichtssendung mit dem LoRa-Modul}
Nachdem wir nun zuverlässige Gewichtsdaten messen können, müssen diese nur noch an die Sammelstelle (den Gateway) gesendet werden. Der Sendevorgang basiert auf dem LoRa-Protokoll, welches vom LoRa-Modul in Empfang und in regelmäßigen Abständen zum LoRa-Gateway gesendet wird (vgl. \ref{fig:Komponentenmodell} Komponentenmodell). Mit Hilfe eines Interrupts, wird jede Sekunde eine Gewichtsmessung durchgeführt. Beim Ablauf des Sendeintervalls (in unserem Projekt momentan eingestellt auf fünfzehn Sekunden), wird in der Menge der Gewichtsmessungen (in unserem Fall fünfzehn Messungen, jede Sekunde eine), nach den letzten fünf Gewichtsmessungen gesucht, die den gleichen Wert haben. Wenn die Messungen diese Bedingung nicht erfüllen, wird kein Wert gesendet.
...
Hier werden alternative Gewichtssensoren aufgezählt und untersucht, um den richtigen Gewichtssensor bei Änderung der Projektanforderungen auswählen zu können.
...
Damit wir überhaupt überprüfen können ob die Daten die wir versenden vollständig und zuverlässig ankommen, benötigen wir zunächst einen Empfänger.
...
Da diese Daten aber verschlüsselt sind, müssen wir zunächst eine Applikation erstellen um lesbare Daten zu erhalten und sie zu einem Sender zuordnen zu können.
...
Bevor wir die LoRa-Nodes mit Strom versorgen, befestigen wir zunächst eine Antenne an den RFH (Radio Frequency High Band – PIN 23) Anschluss der unsere Antenne mit 868 MHz betreiben wird.\\
...
Das Front-End ist die Ansicht, auf welche das Gatronomie-Personal blickt, um die Füllstände aller Behälter zu sehen. Sie ist minimalistisch gehalten, sodass keine Überladung entsteht und das menschliche Auge die relevante Information schneller erkennt. \\
\begin{figure}[H]
    \center
...
Das Things Network ist ein globales, freies und dezentrales Internet der Dinge Netzwerk. Es wurde mit dem Ziel entwickelt, ein weltweit offenes und unabhängiges Internet der Dinge Netzwerk aufzubauen. In unserem konkreten Fall nutzen wir diese Art von Cloud, um lokal erwirtschaftete Daten zu verarbeiten und wieder zur Verfügung zu stellen.\\

\noindent
Der Client sendet über das LoRa-Protokoll Füllstandsdaten an das nächstgelegene Gateway. Das Gateway sendet über das Internet die Daten an TheThingsNetwork, welches dann die Daten an den Webserver schickt. Von da an werden die Daten verarbeitet und in der Visualisierung angezeigt.
...
\begin{itemize}
    ...
    \item \textbf{payload\_raw:} Das Datenpaket von TheThingsNetwork im Hexadezimal ASCII Rohformat
...
Sobald ein Gerät eine Stunde lang keine Nachricht bezüglich einer Gewichtsveränderung geschickt hat, wird es für den Anwender in der visualisierten Webumgebung (kurz UI ~--~ User Interface) als inaktiv gekennzeichnet. Diese Inaktivität würde bei einem Thermobehälter die Abkühlung bedeuten und sollte schnellstmöglich vom Gastronomie-Personal überprüft werden. Diese Zeitbeschränkung wurde nach einer Gesprächsrunde innerhalb der Entwickler festgesetzt.\\
Falls das Gerät nicht in der Lage ist eine Nachricht zu versenden (beispielsweise durch einen unvorhersehbaren Defekt in einem Bauteil), kann das in der Visualisierung für Verwirrung sorgen, da die Messwerte bei Befüllung bzw. Entnahme des Behälterinhalts nicht aktualisert werden. Sobald das Gerät wieder im Netz ist und in der Lage ist Nachrichten zu versenden, wird es wieder als aktiv gekennzeichnet und mit den aktuellen Messwerten im UI eingeblendet. 
...
\newpage
\noindent
Als Mikrocontroller haben wir im Schaltplan einen ATmega644 statt einem ATmega644PA verwendet, da diese fast gleich sind. Ausgehend vom Mikrocontroller werden an den Pins XTAL1 und XTAL2 der Quarz angeschlossen. Die Pins PA0 bis PA7 sind für ein LCD Display reserviert, auf dem Gewichtsinformationen, aber auch Debug-Meldungen ausgegeben werden können. Der Wägesensor ist mit den Pins PD5 und PD6 verbunden. Wobei hier noch die notwendigen Pullup-Widerstände für die I\textsuperscript{2}C- (Two-Wire-) Schnittstelle fehlen. Für das Wägesensor-Bauteil (im Schaltplan fälschlicherweise als Compass, HMC betitelt) wurde ein Platzhalter-Bauteil mit vier Pins verwendet, da kein passendes Layout für unseren Wägesensor gefunden wurde. Das LoRa-Modul wird an PD1 angeschlossen und es wurde ebenso eine Platzhalterkomponente mit drei Pins verwendet. Zudem haben wir einen Taster auf PD0 für die direkte Kommunikation mit dem Wäge-Modul und an PD5 – PD7 einen ISP-Anschluss für die Programmierung angelötet. Die Platine und dessen Bauteile werden mit zwei in Reihe geschalteten Akkus versorgt die jeweils eine Spannung von ca. 3,6 Volt haben und damit eine Gesamtspannung von 7,2 Volt liefern. Dazwischen geschaltet ist ein DC-DC Wandler, der für konstante 5V sorgt.\\
